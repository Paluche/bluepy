import struct
import math
from bluepy.btle import UUID, Peripheral, DefaultDelegate, assigned_numbers
...
class SensorBase:
    sensor_on = struct.pack('B', 0x01)
    sensor_off = struct.pack('B', 0x00)

    def __init__(self, periph, service_uuid, data_uuid, control_uuid):
        self.periph = periph
    ...
    def enable(self):
        ...
        if self.control is None:
            self.control = self.service.get_characteristics(
                               self.control_uuid
                           )[0]
        if self.data is None:
            self.data = self.service.get_characteristics(self.data_uuid)[0]
        if self.sensor_on is not None:
            self.control.write(self.sensor_on, with_response=True)
    ...
    def disable(self):
        if self.control is not None:
            self.control.write(self.sensor_off)


def compute_polynom(coeffs, x):
...
class IRTemperatureSensor(SensorBase):
    zeroC = 273.15  # Kelvin
    tRef = 298.15
    A_POLY = [1.0,      1.75e-3, -1.678e-5]
    B_POLY = [-2.94e-5, -5.7e-7,  4.63e-9]
    C_POLY = [0.0,      1.0,      13.4]
    S0 = 6.4e-14
    ...
    def read(self):
        ...
        S = self.S0 * compute_polynom(self.A_POLY, tDie - self.tRef)
        Vos = compute_polynom(self.B_POLY, tDie - self.tRef)
        fObj = compute_polynom(self.C_POLY, Vobj - Vos)
...
class MovementSensorMPU9250(SensorBase):
    ...
    def enable(self, bits):
        super().enable()
        self.ctrlBits |= bits
        self.control.write(struct.pack('<H', self.ctrlBits))
...
class AccelerometerSensorMPU9250:
    ...
    def read(self):
        """Returns (x_accel, y_accel, z_accel) in units of g"""
        rawVals = self.sensor.raw_read()[3:6]
...
class MagnetometerSensorMPU9250:
    ...
    def read(self):
        ...
        rawVals = self.sensor.raw_read()[6:9]
        return tuple([v * self.scale for v in rawVals])


class BarometerSensor(SensorBase):
    def __init__(self, periph):
        self.calibration_uuid = _ti_uuid(0xAA43)
    ...
    def enable(self):
        super.enable()
        self.calibration = self.service.get_characteristics(
                               self.calibration_uuid
                           )[0]

        # Read calibration data
        self.control.write(struct.pack('B', 0x02), True)
        ...
        self.control.write(struct.pack('B', 0x01), True)

    def read(self):
        """Returns (ambient_temp, pressure_millibars)"""
        (rawT, rawP) = struct.unpack('<hH', self.data.read())
        temp = (self.c1_s * rawT) + self.c2_s
        sens = compute_polynom(self.sensPoly, float(rawT))
        offs = compute_polynom(self.offsPoly, float(rawT))
...
class GyroscopeSensorMPU9250:
    ...
    def read(self):
        ...
        raw_values = self.sensor.raw_read()[0:3]
        return tuple(v * self.scale for v in raw_values)
...
class SensorTag(Peripheral):
    def __init__(self, addr, version=AUTODETECT):
        ...
        if version == AUTODETECT:
            svcs = self.discover_services()
            if _ti_uuid(0xAA70) in svcs:
                version = SENSORTAG_2650
            else:
                version = SENSORTAG_V1

        firmware_version = self.get_characteristics(
            uuid=assigned_numbers.firmwareRevisionString
        )
        if len(firmware_version) >= 1:
            self.firmwareVersion = firmware_version[0].read().decode('utf-8')
        else:
            self.firmwareVersion = u''

        if version == SENSORTAG_V1:
            self.ir_temperature = IRTemperatureSensor(self)
            ...
            self.ir_temperature = IRTemperatureSensorTMP007(self)
...
class KeypressDelegate(DefaultDelegate):
    ...
    def handleNotification(self, hnd, data):
        ...
        button_down = (val & ~self.last_value) & self.ALL_BUTTONS
        if button_down != 0:
            self.on_button_down(button_down)
        button_up = (~val & self.last_value) & self.ALL_BUTTONS
        if button_up != 0:
            self.on_button_up(button_up)
        self.last_value = val

    def on_button_up(self, but):
        print('** ' + self._button_desc[but] + ' UP')

    def on_button_down(self, but):
...
def main():
    ...
    if arg.keypress or arg.all:
        tag.keypress.enable()
        tag.set_delegate(KeypressDelegate())
    ...
    # Some sensors (e.g., temperature, accelerometer) need some time for
    # initialization.
    # Not waiting here after enabling a sensor, the first read value might be
    # empty or incorrect.
    time.sleep(1.0)
    ...
    while True:
        if arg.temperature or arg.all:
            print('Temp: ', tag.ir_temperature.read())
        ...
        counter += 1
        tag.wait_for_notifications(arg.t)
