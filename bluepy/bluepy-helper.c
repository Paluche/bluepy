...
static void events_handler(const uint8_t *pdu,
                           uint16_t       len,
                           gpointer       user_data)
{
    uint8_t * opdu;
...
static void gatts_find_info_req(const uint8_t * pdu,
                                uint16_t        len,
                                gpointer        user_data)
{
    uint8_t * opdu;
...
static void gatts_find_by_type_req(const uint8_t * pdu,
                                   uint16_t        len,
                                   gpointer        user_data)
{
    uint8_t * opdu;
...
static void gatts_read_by_type_req(const uint8_t * pdu,
                                   uint16_t        len,
                                   gpointer        user_data)
{
    uint8_t * opdu;
...
static void gatts_read_req(const uint8_t * pdu,
                           uint16_t        len,
                           gpointer        user_data)
{
    uint8_t * opdu;
...
static void gatts_read_blob_req(const uint8_t * pdu,
                                uint16_t        len,
                                gpointer        user_data)
{
    uint8_t * opdu;
...
static void gatts_read_multi_req(const uint8_t * pdu,
                                 uint16_t        len,
                                 gpointer        user_data)
{
    uint8_t * opdu;
...
static void gatts_read_by_group_req(const uint8_t * pdu,
                                    uint16_t        len,
                                    gpointer        user_data)
{
    uint8_t * opdu;
...
static void gatts_write_req(const uint8_t * pdu,
                            uint16_t        len,
                            gpointer        user_data)
{
    uint8_t * opdu;
...
static void gatts_write_cmd(const uint8_t * pdu,
                            uint16_t        len,
                            gpointer        user_data)
{
    assert(len >= 3);
...
static void gatts_signed_write_cmd(const uint8_t * pdu,
                                   uint16_t        len,
                                   gpointer        user_data)
{
    assert(len >= 15);
...
static void gatts_prep_write_req(const uint8_t * pdu,
                                 uint16_t        len,
                                 gpointer        user_data)
{
    uint8_t * opdu;
...
static void gatts_exec_write_req(const uint8_t * pdu,
                                 uint16_t        len,
                                 gpointer        user_data)
{
    uint8_t *opdu;
...
static void gatts_mtu_req(const uint8_t *pdu, uint16_t len, gpointer user_data)
{
    ...
    // According to the Bluetooth specification, we are supposed to send the
    // response before applying the new MTU value:
    //   This ATT_MTU value shall be applied in the server after this response
    //   has been sent and before any other Attribute protocol PDU is sent.
...
static void primary_all_cb(uint8_t status, GSList * services, void * user_data)
{
    ...
    {
        struct gatt_primary *prim = service->data;
...
static void primary_by_uuid_cb(uint8_t status, GSList *ranges, void *user_data)
{
    ...
    resp_begin(rsp_DISCOVERY);
    for (service = ranges; service; service = service->next)
    {
        struct att_range *range = service->data;
...
static void included_cb(uint8_t status, GSList *includes, void *user_data)
{
    ...
    resp_begin(rsp_DISCOVERY);
    for (service = includes; service; service = service->next)
    {
        struct gatt_included *incl = service->data;
...
static void char_cb(uint8_t status, GSList *characteristics, void *user_data)
{
    ...
    resp_begin(rsp_DISCOVERY);
    for (service = characteristics; service; service = service->next)
    {
        struct gatt_char *chars = service->data;
...
static void char_desc_cb(uint8_t status, GSList *descriptors, void *user_data)
{
    ...
    for (service = descriptors; service != NULL; service = service->next)
    {
        struct gatt_desc *desc = (struct gatt_desc *)service->data;
...
static void char_read_cb(guint8         status,
                         const guint8 * pdu,
                         guint16        plen,
                         gpointer       user_data)
{
...
static void char_read_by_uuid_cb(guint8         status,
                                 const guint8 * pdu,
                                 guint16        plen,
                                 gpointer       user_data)
{
...
static gboolean channel_watcher(GIOChannel * chan,
                                GIOCondition cond,
                                gpointer     user_data)
{
    DBG("chan = %p", chan);
...
static void cmd_connect(int argcp, char ** argvp)
{
    ...
    iochannel = gatt_connect(opt_src,
                             opt_dst,
                             opt_dst_type,
                             opt_sec_level,
                             opt_psm,
...
static void char_write_req_cb(guint8         status,
                              const guint8 * pdu,
                              guint16        plen,
                              gpointer       user_data)
{
...
static void exchange_mtu_cb(guint8         status,
                            const guint8 * pdu,
                            guint16        plen,
                            gpointer       user_data)
{
...
static void set_mode_complete(uint8_t      status,
                              uint16_t     length,
                              const void * param,
                              void *       user_data)
{
...
static void add_remote_oob_data_complete(uint8_t      status,
                                         uint16_t     len,
                                         const void * param,
                                         void *       user_data)
{
...
static bool add_remote_oob_data(uint16_t         index,
...
static void cmd_add_oob(int argcp, char ** argvp)
{
    ...
    if ((!memcmp(argvp[3], "C_192", 5)) && (!memcmp(argvp[5], "R_192", 5)))
    {
        ...
        R192 = argvp[6];
        if ((argcp > 8) &&
            (!memcmp(argvp[5], "C_256", 5)) &&
            (!memcmp(argvp[7], "R_256", 5)))
        {
            C256 = argvp[6];
            R256 = argvp[8];
        }
    }
    else if ((!memcmp(argvp[3], "C_256", 5)) &&
             (!memcmp(argvp[5], "R_256", 5)))
    {
        C256 = argvp[4];
        R256 = argvp[6];
    }
...
static void read_local_oob_data_complete(uint8_t      status,
                                         uint16_t     len,
                                         const void * param,
                                         void       * user_data)
{
...
static void pair_device_complete(uint8_t      status,
                                 uint16_t     length,
                                 const void * param,
                                 void *       user_data)
{
...
static void cmd_pair(int argcp, char ** argvp)
{
    ...
    if (res == 0)
    {
        DBG("mgmt_send(MGMT_OP_PAIR_DEVICE) failed for %s for hci%u",
            opt_dst,
            mgmt_ind);
...
static void unpair_device_complete(uint8_t      status,
                                   uint16_t     length,
                                   const void * param,
                                   void *       user_data)
{
...
static void cmd_unpair(int argcp, char ** argvp)
{
    ...
    if (res == 0)
    {
        DBG("mgmt_send(MGMT_OP_UNPAIR_DEVICE) failed for %s for hci%u",
            opt_dst,
            mgmt_ind);
        resp_mgmt(err_SEND_FAIL);
        return;
...
static void scan_cb(uint8_t      status,
                    uint16_t     length,
                    const void * param,
                    void *       user_data)
{
...
// Unlike Bluez, we follow BT 4.0 spec which renamed Device Discovery by Scan.
static void scan(bool start)
{
    ...
    opcode  = start ? MGMT_OP_START_DISCOVERY : MGMT_OP_STOP_DISCOVERY;
...
static gboolean hci_monitor_cb(GIOChannel  *chan,
                               GIOCondition cond,
                               gpointer     user_data)
{
    ...
    r = g_io_channel_read_chars(chan, (gchar *) buf, 1, &len, &err);

    if (r != G_IO_STATUS_NORMAL)
    {
        if (err)
        {
          DBG("reading pkt type reports state %d: %s", r, err->message);
        }
    ...
    switch (type)
    {
        ...
        {
            hci_command_hdr *ch;
            r = g_io_channel_read_chars(chan,
                                        (gchar *) buf,
            ...
            if (r != G_IO_STATUS_NORMAL)
            {
                if (err)
                {
                    DBG("g_io_channel_read_chars() reports state %d: %s",
                        r,
                        err->message);
                }
                return true;
            ...
            switch (ch->opcode)
            {
                ...
                {
                    le_set_scan_enable_cp *lescan;

                    lescan = (le_set_scan_enable_cp *) ptr;
        ...
        case HCI_EVENT_PKT:
        {
            ...
            switch (eh->evt)
            {
                case EVT_CMD_COMPLETE:
                {
                    ...
                    // DBG("command complete (0x%02x|0x%04x) 0x%02x 0x%02x",
                    //     cmpl->ncmd,
                    //     cmpl->opcode,
                    //     *(uint8_t *)(ptr+3),
                    //     *(uint8_t *)(ptr+4));
                ...
                case EVT_LE_META_EVENT:
                {
                    ...
                    switch (meta->subevent)
                    {
                        case EVT_LE_ADVERTISING_REPORT:
                        {
                            ...
                            // DBG("Device found: "
                            //     "%02X:%02X:%02X:%02X:%02X:%02X type=%X "
                            //     "length=%d data[0]=0x%02x rssi=0x%02x",
                            //     val[5],
                            //     val[4],
                            //     val[3],
                            //     val[2],
                        ...
                        default:
                        {
                            DBG("Ignoring EVT_LE_ADVERTISING_REPORT subevent "
                                "%02x",
                                meta->subevent);
                            return true;
                        }
...
// Perform a passive scan, i.e. report ADV_IND packets but do not request
// SCN_RSP packets.
...
static struct {
...
} commands[] = {
    {
        ...
        .cmd    = "conn",
        .func   = cmd_connect,
        .params = "[address [address type [interface]]]",
        ...
        .desc   = "Disconnect from a remote device" },
    {
        ...
        .func   = cmd_le,
        .params = "[on | off]",
        .desc   = "Control LE feature on the controller"
    }, {
        .cmd    = "remote_oob",
        .func   = cmd_add_oob,
        .params = "address [[C_192 c192] [R_192 r192]] [[C_256 c256] "
                  "[R_256 r256]]",
        .desc   = "Add OOB data for remote address"
    }, {
        .cmd    = "local_oob",
        .func   = cmd_read_oob,
        .params = "",
        .desc   = "Read local OOB data"
    }, {
        .cmd    = "pairable",
        .func   = cmd_pairable,
        .params = "[on | off]",
        .desc   = "Control PAIRABLE feature on the controller"
    }, {
        .cmd    = "pair",
        .func   = cmd_pair,
        .params = "",
        .desc   = "Start pairing with the device"
    }, {
        .cmd    = "unpair",
        .func   = cmd_unpair,
        .params = "",
        .desc   = "Start unpairing with the device"
    }, {
        .cmd    = "scan",
        .func   = cmd_scan,
        .params = "",
        ...
        .func   = cmd_pasv,
        .params = "",
        .desc   = "Start passive scan"
    }, {
        .cmd    = "pasvend",
        .func   = cmd_pasvend,
        .params = "",
        .desc   = "Force passive scan end"
    }, {
        .cmd    = NULL,
        .func   = NULL,
        .params = NULL,
...
static gboolean prompt_read(GIOChannel * chan,
                            GIOCondition cond,
                            gpointer     user_data)
{
    gchar *   myline;
    ...
    g_io_status = g_io_channel_read_line(chan, &myline, NULL, NULL, NULL);
    if ((g_io_status != G_IO_STATUS_NORMAL) || (myline == NULL))
    {
...
static void read_version_complete(uint8_t      status,
                                  uint16_t     length,
                                  const void * param,
                                  void       * user_data)
{
...
static void mgmt_device_connected(uint16_t     index,
                                  uint16_t     length,
                                  const void * param,
                                  void *       user_data)
{
...
static void mgmt_scanning(uint16_t     index,
                          uint16_t     length,
                          const void * param,
                          void       * user_data)
{
...
static void mgmt_device_found(uint16_t     index,
                              uint16_t     length,
                              const void * param,
                              void *       user_data)
{
    ...
    // DBG("Device found: %02X:%02X:%02X:%02X:%02X:%02X type=%X flags=%X",
    //     val[5],
    //     val[4],
    //     val[3],
    //     val[2],
...
static void mgmt_setup(unsigned int idx)
{
    ...
    if (!mgmt_master)
    {
        DBG("Could not connect to the BT management interface, try with su "
            "rights");
        return;
    }
...
int main(int argc, char ** argv)
{
    ...
    printf("# " __FILE__ " version " VERSION_STRING " built at " __TIME__
           " on " __DATE__ "\n");
